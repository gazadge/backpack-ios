...
@interface BPKTappableLinkLabel
...
- (void)setupWithStyle:(BPKFontStyle)style {
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [self.heightAnchor constraintGreaterThanOrEqualToConstant:140.0f]
    ]];
}
...
- (void)updateTextColors {
    ...
    self.contentView.linkAttributes = @{
        NSForegroundColorAttributeName: [BPKColor blend:self.linkColor with:BPKColor.white weight:0.85f],
...
- (void)updateTextDisplay {
    ...
    NSAttributedString *newString = [[NSAttributedString alloc] initWithString:displayText
                                                                    attributes:defaultAttributes];

    self.contentView.text = newString;

    // Re-apply the links
    for (int i = 0; i < [_persistedLinks count]; i += 1) {
        BPKTappableLinkDefinition *linkDefinition = [_persistedLinks objectAtIndex:i];
